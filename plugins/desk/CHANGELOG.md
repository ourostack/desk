...
Adds the Claude Code deriver, the factory's first source reader: `factory/derive-claude.js` streams a session's native JSONL transcript line by line and turns it into a facts object (`jobs: []`, filled in by a later task) plus in-memory binding events for matching Desk tool calls, file writes and commits to tasks. It deduplicates the usage Claude Code repeats across streamed lines of one message, buckets every tool call and outcome, infers turns and human waits from prompt and activity timestamps (ignoring hook-injected lines), folds one level of subagent transcripts in as their own numbered agents, and dedupes PR refs seen through both `pr-link` and `gitOperation.pr`. Nothing it derives ever carries transcript text, prompt text, tool input or tool output — those only ever reach the in-memory events, never the facts. Commits are not native to the host, so `refs.commits` stays empty with `unavailable {commits, host_does_not_record}`; permission waits and CI runs are unavailable for the same reason and by slice-1 scope. Ships its own generator, `__tests__/factory/fixtures/claude/make.js`, which builds the synthetic transcripts the deriver's tests run against — no real transcript is read or copied. Also extends `schema.test.js`'s oversized-array cases to plant the sentinel inside every capped array's unread items, not only `plugins`, closing a gap noted in the facts-schema review. Ships `desk-mcp@1.4.0-alpha.6` source; the MCP version and runtime packs are unchanged.
