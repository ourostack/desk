...
Adds the factory's Copilot CLI deriver (`factory/derive-copilot.js`): it streams one session's `events.jsonl` in a single pass with small state and turns it, with that session's rows in the host's `session-store.db`, into privacy-safe local facts (turns, tool calls and outcomes, human and permission waits, subagents, API retries, compactions, usage, plugins, PR and commit references) plus in-memory binding events for Desk task tools and successful file writes. Usage comes from the last `session.shutdown` when one exists and otherwise from the database rows, never both; no content field is read into facts, and every copied value is validated, so the output always passes `validateFacts`. The Copilot usage reader moves to `factory/copilot-usage.js` and now opens the database with `node:sqlite`, loaded only when a read happens so the MCP server still starts on Node 20; `measurement/copilot-usage.js` re-exports it unchanged for the work ledger. Nothing calls the deriver yet. Ships `desk-mcp@1.4.0-alpha.6` source; the MCP version and runtime packs are unchanged.
