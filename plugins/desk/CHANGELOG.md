...
## 3.2.0-alpha.31 — 2026-09-25

Milestone 3a, task A2: handshake first, then admission. The Desk MCP server now answers `initialize` and `tools/list` before it does anything that can fail or take time, and every failure after that is a named degraded state that recovers in the same session, with no restart. Before the handshake, `index.js` only checks the Node version and moves to a compatible Node when no shipped runtime pack fits; the runtime-pack restore, root resolution, the activation config and its policy, write authority, the new state-branch check and the readiness-controller election all run afterwards as background admission (`src/runtime/admission.js`, `src/runtime/desk-session.js`) behind a dependency-free front door (`src/runtime/front-door.js`) that always lists all 18 tools. Admission is `admitting`, then `ready` or `degraded:<code>`; a degraded session retries after 1, 2, 5, 10 and 30 s and then every 60 s, and at once on every `desk_status` call and on a `.git/HEAD` change, so creating a missing desk, fixing an activation config or pushing local commits upgrades the running session to `ready`. Reads work without a readiness controller (lexical search and timeline read the files directly); writes need admitted authority, a controller and the checkout on its state branch, checked again before every write; a refused tool returns `{status: "degraded", code, fix, blockers}`. A ready session checks its controller every 60 s and before writes, and re-elects a lost one in the background; a dead owner's socket, or a socket nobody listens on (even with a corrupt `owner.json`), is reclaimed, and a readiness state directory with a loose mode is tightened to 700. New generic `--state-branch <name>` (or `desk.state_branch` in the activation config): a detached HEAD, or a branch equal to its upstream, is switched back with `git switch` only when the tracked tree is clean, no Git operation or `index.lock` is in progress, no commit is local-only and the state branch exists; Desk never fetches or resets, and reports `repaired: detached HEAD → main (was <sha>)`. Other off-branch states stay read-only with a precise fix, and `desk_doctor {"repair":"switch_state_branch"}` covers a pushed branch with no upstream. `desk_doctor {"repair":"prune_readiness_state"}` removes leftover readiness-controller folders whose owner is dead and whose root is gone. Desk records each state change in `last-start.json` (`state`, `code`, `repair`) and each repair in `repairs.log` in its state directory. An unreadable index database is reported as `corrupt` by `desk_status` and moved aside and rebuilt during convergence. A1's bounded pre-handshake admission retry is removed. The Claude `.mcp.json` launcher now falls back to its own responder when the bootstrap's `run()` rejects, not only when it throws, and remediation text names the in-session step instead of restarting the host. Tests pass a temporary `stateHome` and never write to the real `~/.cache` or `~/.local/state`. Ships `desk-mcp@1.4.0-alpha.6` source; the MCP version and runtime packs are unchanged.
