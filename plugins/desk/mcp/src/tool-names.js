...
export const TOOL_DESCRIPTIONS = {
  ...
  track_rename:
    "Rename a track — `to` must be a valid track name per `validateTrackName`, the same rule `track_create` enforces (rejects a prompt-copied, credential-like, catch-all, or person name). Refuses if the target already exists. Rewrites `track:` in every task.md under the moved tree, live and archived. Stages the move with `git mv` on a Git desk, a plain rename otherwise; never commits. On a Git desk it refuses a track with uncommitted changes or untracked, non-ignored files, because another session may be working there, unless `allow_dirty: true`; the refusal never quotes names. Returns `mentions`: other .md files under the desk that still reference the old track path in free text — reported, never rewritten.",
  friction_add:
