...
export const TOOL_DESCRIPTIONS = {
  ...
  desk_doctor:
    "Report whether Desk MCP started in healthy runtime mode and describe the active runtime target. In diagnostic mode, reports the precise startup failure and offline remediation. Optional format:'preview' returns only a local-on-demand, nine-field package/process snapshot with no task or feedback records. Optional repair:'switch_state_branch' switches the desk checkout back to its state branch when that is safe (clean tracked tree, no Git operation in progress, no local-only commits); repair:'reclaim_controller' stops a readiness controller that accepts connections but has not answered 3 probes in a row, when its process is provably this user's Desk, and re-elects one; repair:'prune_readiness_state' removes leftover readiness-controller folders whose owner process is dead and whose root no longer exists.",
}
