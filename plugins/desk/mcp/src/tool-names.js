...
export const TOOL_DESCRIPTIONS = {
  ...
  desk_work_ledger:
    "Private work-item measurement about the caller's own work, stored in the OS user's own state directory — never in the desk Git workspace, the search index, or telemetry. A work item is one request for one specific independently assessable outcome; identity is taken at intake before any commitment. Records commitment, size features (before execution), phases, scope changes, links, completion and closure; and records an exact file/directory scope envelope plus a one-ruling-per-cycle work-design loop through `work_contract` (`run_contract`), `work_cycle` (`cycle`), and `work_ruling` (`work_design_ruling`). Detects repeated boundary rejection, stalled findings without new learning, write-set escape, and declared architecture expansion; blocks a new cycle while a pivot is unresolved; leaves the ruling model-owned; keeps the stored discriminator strict; canonicalizes legacy comparison on read without rewriting history; imports minimal usage facts with provenance from the host's own local session records; and reports every field as measured, declared, inferred, estimated or unavailable. Actions: `capabilities`, `intake`, `commit`, `size`, `phase`, `scope_change`, `link`, `complete`, `close`, `correct`, `delete`, `inspect`, `report`, `run_contract`, `cycle`, `work_design_ruling`, `import_usage`, `cost_basis`, `set_recording`, `link_evaluation_receipt`. Scoped to the session's desk root and --person binding; it cannot read or write another participant's ledger. Results return to this caller only — there is no share or export action. It measures work, not people: no ranking, no scoring, no transcripts, and no universal credit-to-money conversion.",
  desk_search:
