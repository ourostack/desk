...
test("Copilot root evidence and support matrix record flattened packaging as generated", () => {
  ...
  assert.equal(
    ...
    "load the generated flattened Desk, Work Suite, Plain Language, and Ponytail bundle metadata",
  )
...
test("Copilot packaging validation rejects missing root surfaces and stale versions", () => {
  ...
  assert.deepEqual(
    ...
    ["Copilot root Desk version must match activation version 2.1.1"],
  )
  ...
  assert.deepEqual(
    ...
    ["Copilot root Work Suite version must match activation lock 2.1.1"],
  )
  ...
  assert.deepEqual(
    ...
    ["Copilot root Plain Language version must match activation lock 0.1.1"],
  )
