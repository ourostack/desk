...
// where the violation is string-shaped, and asserts the sentinel never
// appears in the serialized errors — errors are `{ code, path }` only, never
// the offending value.
...
import { validateFacts, validateFactsBytes, ENUMS, PATTERNS, LIMITS } from "../../src/factory/schema.js"
...
test("intervals[].tool is forbidden when kind is not tool (surfaces as an unrecognized key)", () => {
...
test("an unrecognized key in counts.tool_calls fails with unknown_key naming the map, not the key", () => {
  const value = golden()
  value.counts.tool_calls.not_a_tool_kind = 1
  const result = validateFacts(value)
...
// --- jobs[].basis: non-empty subset of ENUMS.jobBasis -----------------------
...
// --- Oversized arrays: too_many ---------------------------------------------

test("more than 64 plugins fails with too_many", () => {
  const value = golden()
  value.plugins = new Array(LIMITS.plugins + 1).fill({ name: "desk", version: "1.0.0" })
  assertSingle(validateFacts(value), "too_many", "plugins")
})
...
// --- unknown_key at every object level --------------------------------------
...
for (const level of UNKNOWN_KEY_LEVELS) {
  test(`an unknown key at ${level.label} fails with unknown_key naming the container`, () => {
    const value = golden()
    const target = level.keys.length === 0 ? value : at(value, [...level.keys, "x"])
    target.zzz_unexpected = SENTINEL
    const result = validateFacts(value)
...
for (const keys of OBJECT_TYPE_CASES) {
  test(`${ps(keys)} of the wrong type fails with type`, () => {
    const result = validateFacts(setPath(golden(), keys, 42))
...
test("jobs[].observed of the wrong non-null type fails with type", () => {
  const result = validateFacts(setPath(golden(), ["jobs", 0, "observed"], "not-observed"))
...
test("ENUMS, PATTERNS and LIMITS are frozen and cover every documented enum", () => {
  assert.ok(Object.isFrozen(ENUMS))
  assert.ok(Object.isFrozen(ENUMS.toolKind))
  assert.ok(Object.isFrozen(PATTERNS))
  assert.ok(Object.isFrozen(LIMITS))
  assert.deepEqual(ENUMS.toolKind, ["read", "edit", "shell", "search", "web", "agent", "desk", "skill", "mcp", "plan", "other"])
})
