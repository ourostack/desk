...
test("a Desk session releases its controller on every normal end: stdin closed, SIGTERM and SIGINT leave no owner record or socket file", { skip: posixOnly, timeout: 120000 }, async (t) => {
  ...
  for (const end of ["stdin", "SIGTERM", "SIGINT"]) {
...
test("a stale owner record whose PID now belongs to an unrelated process: other sessions reclaim the controller and reach ready", { skip: posixOnly, timeout: 120000 }, async (t) => {
  ...
  for (const session of sessions) t.after(() => session.close())
  for (const session of sessions) {
    const status = await session.statusUntil((payload) => payload.state === "ready", { deadlineMs: 60000, intervalMs: 300 })
  ...
  assert.equal(recall.isError, false, JSON.stringify(recall.payload))
  const write = await sessions[1].call("task_create", { track: "ops", slug: "after-pid-reuse", title: "Reclaimed" })
  assert.equal(write.isError, false, JSON.stringify(write.payload))
  assert.equal(unrelated.exitCode, null)
  assert.equal(unrelated.signalCode, null, "the unrelated process was never signalled")
})
