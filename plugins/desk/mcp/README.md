...
- `desk_doctor` — healthy-runtime confirmation or precise failure diagnosis and remediation; `{"repair":"switch_state_branch"}` and `{"repair":"prune_readiness_state"}` run the repairs described under [Handshake first, then admission](#handshake-first-then-admission)
...
Everything else runs afterwards as background admission (`src/runtime/desk-session.js`, `src/runtime/admission.js`): root resolution, the activation config and its `desk_runtime` policy, the runtime-pack restore, the state-branch check, write authority and the readiness-controller election. Its state is `admitting`, then `ready` or `degraded:<code>`:

- A degraded session retries on its own after 1, 2, 5, 10 and 30 s, then every 60 s. Every `desk_status` call retries at once and waits up to 3 s for the answer. A change to the checkout's `.git/HEAD` also retries. So a fix the agent makes in the session (creating the desk, correcting the activation config, pushing local commits) upgrades the same session to `ready` with no restart.
- A ready session checks every 60 s, and before every write, that its readiness controller still answers. A lost controller is re-elected in the background; a dead owner's stale socket, or a socket nobody listens on, is reclaimed.
- `desk_status` always answers, with `state`, `code`, `fix`, the latest `repair` and an `admission` block (attempts, next retry, state branch, whether writes are available).
...
- `desk_reindex` and `desk_work_ledger` also need admitted authority and a controller. Writes (`task_*`, `track_*`, `friction_add`, `lesson_add`) also need the checkout on its state branch, checked again right before each write.
- A tool whose needs are not met returns `{"status":"degraded","state","code","fix","blockers","tool"}` with a fix the agent can act on in the session.

Desk records each state change in `last-start.json` (`state`, `code`, `repair`, `fix`) in its state directory, `$XDG_STATE_HOME/ouroboros-skills/desk` or `~/.local/state/ouroboros-skills/desk`, and appends each repair it makes to `repairs.log` there. Tests and embedders pass `stateHome` to `main()` to keep both, and the readiness controllers, in a temporary folder.

### The state branch

A host that keeps the desk on one branch passes `--state-branch <name>` (or sets `desk.state_branch` in the activation config). Desk then checks the checkout that holds the desk root. When HEAD is elsewhere, Desk runs `git switch <name>` only when all of these hold: no tracked change, no merge, cherry-pick, revert, bisect or rebase in progress, no `index.lock`, no local-only commits (HEAD is on a remote branch, or the branch equals its upstream), and the local state branch exists. It never fetches or resets, and `git switch` keeps untracked files.

- A detached HEAD, and a branch that equals its upstream, are switched back automatically. Desk reports `repaired: detached HEAD → main (was <sha>)` in `desk_status`, `last-start.json` and `repairs.log`.
- A branch with no upstream whose commits are all on a remote branch is safe but may be in use on purpose, so the agent asks for it: `desk_doctor {"repair":"switch_state_branch"}` runs the same preconditions.
- Anything else (tracked changes, an operation in progress, local-only commits) stays as it is. Reads keep working, writes return `degraded:state_branch_detached` or `degraded:state_branch_mismatch` with the blockers and the fix, for example pushing the local commits, after which the next retry switches back.

### Other repairs

- A readiness state directory of ours with a looser mode than `700` is tightened to `700`.
- An unreadable index database (truncated, or not a database) is moved aside and rebuilt during convergence; `desk_status` reports it as `local_db.state: "corrupt"` meanwhile.
- `desk_doctor {"repair":"prune_readiness_state"}` removes leftover readiness-controller folders whose owner process is dead and whose root no longer exists.
