...
- **Hung controllers.** A controller that accepts connections but does not answer a handshake probe within 5 s (`DESK_READINESS_PROBE_MS`) is a miss. After 3 misses in a row, spaced by the admission backoff, the session is `degraded:controller_hung`: it stays controller-free, reads and writes keep working, and Desk keeps retrying, so it recovers when the controller answers again or its session ends. Desk never signals the owner, because today the controller runs inside another session's Desk MCP server and stopping it would cost that session its Desk connection. `desk_doctor {"repair":"reclaim_controller"}` reports the owner PID and endpoint and reclaims nothing; the controller becomes reclaimable once it runs in a child process of its own (task A2b).
