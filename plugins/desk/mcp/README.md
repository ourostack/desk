...
- **A child process, not an MCP session.** The electing session starts the controller, watcher, journal and index writer on a separate event loop in its own child process. Reindexing cannot monopolize the owning MCP session's event loop. The [6,000-document regression](__tests__/runtime/controller_process.test.js) rebuilds a missing index while measuring every owning-session `tools/list` reply against the 200 ms bound. As with admission, this is a normal-operation bound, not a guarantee on a machine the OS cannot schedule.
