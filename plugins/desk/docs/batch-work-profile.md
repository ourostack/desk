...
`value_adding` requires the profile's own `outcome.status` to be `"accepted"` — an accepted endpoint criterion — and at least one cited evidence entry with `claim_type: "outcome"`, `producer: "independent_evaluator"`, and `class: "measured"`. A declared producer label alone, an `unavailable`-class entry, or a self-declared outcome with no qualifying independent-evaluator reference, all refuse; the independent verification of that reviewer identity remains a later consumer's job. This schema does not add `mura` or `muri` fields: neither a single observation nor a large token count can establish either, and no automatic derivation from usage or activity counts is performed. Lean interpretation here is annotation, not measured truth, and it never resets or upgrades the declared binding identity.
