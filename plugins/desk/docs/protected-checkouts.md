...
The [dependency-free inspector](../mcp/src/runtime/shell-commands.js) tokenizes commands without executing them. It follows quoted and escaped words, `cd`, repeated `git -C`, Git directory options and environment assignments, command lists, `&&`/`||`, pipelines, background commands, groups, conditionals, literal loops and functions, shell wrappers and positional arguments, Git aliases, substitutions and here-documents. Literal quoted text and quoted here-document bodies are not treated as commands. Unknown external exit statuses retain both possible conditional paths. The [edge tests](../mcp/__tests__/runtime/protected_checkout_edges.test.js) cover policy failures and malformed inputs as well as ordinary allowed commands.
