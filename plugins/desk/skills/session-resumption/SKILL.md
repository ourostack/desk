...
- The actual host/process generation, remaining delegated writers and the host-owned recovery entry point, with its availability limits. Keep the exact owned resources and their dispositions in the existing Markdown resources table through `desk:task-lifecycle`.
