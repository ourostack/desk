...
The ID goes in the entry's heading, and once the entry is published it never changes. A correction keeps it; a withdrawal keeps it. Do not renumber, reuse, or tidy published IDs — not even the ID of a withdrawn entry — because the sequence records what was written, not what survives. A draft that has not landed in the file yet is not a record: if its ID collides during publication, it gets a new one before it lands, as below.

Allocation is conflict-aware, because rereading a file and then writing it is a race, not a guarantee: two sessions can both read the same free `NN` before either appends, and a second checkout cannot see an entry that has not been merged yet.

So publish against the authoritative file, not a local snapshot. Take the desk's exclusive write path when it has one — a lock, an atomic compare-and-write, or an exclusive create — and hold it across choosing `NN` and appending. When the desk has none, use its Git contribution path as the authority: refresh `_meta/preview-feedback.md` from the branch of record immediately before writing, verify the chosen `NN` is still unused there, append, and publish. If publication is rejected, or the refresh or merge shows that ID already taken, the unpublished entry does not keep it: recompute the sequence against the refreshed file, show the participant the final entry again, and retry. Repeat until it lands — a losing writer retries; it never overwrites, force-publishes, or quietly reuses the ID.
