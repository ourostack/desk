...
7. Record and commit. Write _meta/organization.json in this session's own desk with: $record
   Fix the files that task_move and track_rename reported under mentions when they matter. Run desk_doctor to confirm the Organization section. Before committing, check git diff --cached --name-status: it must hold only the tidy's moves, renames, the files you fixed and the record. If anything else is staged, unstage it, leave the tidy uncommitted and stop with one line saying so. Otherwise commit only those paths, with a message that lists every move and rename by its new name, then push.
