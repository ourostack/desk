...
This candidate publishes all three offline runtime packs, each built on its own platform and Node ABI rather than relabelled from another host: ARM macOS on Node 22 (ABI 127) from the maintainer's machine, Linux x64 on Node 22.23.2 (ABI 127) from the Ubuntu runner, and Windows x64 on Node 24.18.0 (ABI 137) from the Windows runner. Each pack is verified where it was built — its own native SQLite addon and vector extension are loaded under the Node runtime it claims — and again in this repository against its manifest, checksum and declared platform and architecture. Earlier candidates' packs remain in place for their own versions. Use the matching Node version on the CLI's `PATH`. The [native Windows CI](https://github.com/ourostack/ouroboros-skills/actions/runs/34266621708) exercised actual NTFS protection, feedback CRUD/reopen and offline source-mirror attribution on an earlier candidate; it does not qualify a full Windows CLI installation or skill discovery here. A different Node/architecture combination is not qualified by these packs.
