...
The same metadata-only re-anchor ran again when the MCP package's `test` script began preloading the test isolation setup (`--import ./__tests__/_isolated_env.mjs`): the published snapshot was decompressed into a scratch desk index and both builders were re-run with `--from-local-db` and the same ids and provenance commit. No indexing or embedding request was made, the pack `.jsonl`, the snapshot `.sqlite.zst` and both checksums are byte-identical, and only `artifact_source_scope_hash` and `created_at` moved.
